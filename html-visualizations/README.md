...
- **⭐ All Node POV visualizations** drop the entire output folder produced by the Node POV recipes script (the `.moderne-pov-node` directory). Auto-detects the JavaScript / TypeScript dependency-insight, vulnerability-check, language-composition, ECMAScript best-practices, and Node version upgrade tables, plus the awk-built `roi_summary.csv`, `roi_per_kloc.csv`, `recipe_files.csv`, and `language_composition_summary.csv` rollups, and renders 18 Node-relevant visualizations grouped into Inventory, Dependencies (NPM violin, top packages by install count, direct vs transitive, license distribution, version drift, vulnerabilities), Recipe outcomes (composite Sankey, recipe-impact heatmap, time-saved heatmap, files per recipe), ROI (hours saved, hours per kLOC), and Modernization (Node version distribution + per-repo).
