...
def dataframe_to_tree(df):
    ...
    df["versionConstraint"].fillna("", inplace=True)

    tree = ud.UniqueDictionaries()

    for _, row in df.iterrows():
        parent_path = row["repositoryPath"]
        child_name = row["packageName"]

        tree.add(
            {
                "path": (parent_path, child_name),
                "version": f"{row['versionConstraint']}",
            }
        )
...
def create_tree_view():
    ...
    df = df.drop_duplicates(subset=["repositoryPath", "packageName"])
