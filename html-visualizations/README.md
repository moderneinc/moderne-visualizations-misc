...
- **⭐ All Node POV visualizations** drop the entire output folder produced by the Node POV recipes script (the `.moderne-pov-node` directory). Auto-detects the JavaScript / TypeScript dependency-insight, vulnerability-check, language-composition, ECMAScript best-practices, and Node version upgrade tables, plus the awk-built `roi_summary.csv`, `roi_per_kloc.csv`, `recipe_files.csv`, and `language_composition_summary.csv` rollups, plus every `UpgradeDependencyVersion<Library>_SourcesFileResults.csv` and `UpgradeTransitiveDependencyVersion<Library>_SourcesFileResults.csv` (library name parsed from filename — drop a folder with new library files and they appear automatically as new bars / heatmap columns). Renders 20 Node-relevant visualizations grouped into Inventory, Dependencies (NPM violin, top packages by install count, direct vs transitive, license distribution, version drift, vulnerabilities), Recipe outcomes (composite Sankey, recipe-impact heatmap, time-saved heatmap, files per recipe), ROI (hours saved, hours per kLOC), Modernization (Node version distribution + per-repo), and NPM library upgrades (per-library bar with direct vs transitive, repo × library heatmap; libraries whose recipe ran but produced no findings render as zero-bars).
...
| ⭐ All Node POV visualizations | `node_pov_all` | Drop the whole `.moderne-pov-node` folder produced by the Node POV recipes script; auto-detects `NodeDependenciesInUse`, `VulnerabilityReportNpm`, `LanguageCompositionPer{File,Folder,Repository}`, `SourcesFileResults`, `UpgradesAndMigrations`, `SecurityIssues`, plus the awk-built `roi_summary.csv` / `roi_per_kloc.csv` / `recipe_files.csv` / `language_composition_summary.csv` rollups, plus every `UpgradeDependencyVersion<Library>_SourcesFileResults.csv` / `UpgradeTransitiveDependencyVersion<Library>_SourcesFileResults.csv` (library names parsed from filename, no per-library code changes needed when new libraries are added) |
